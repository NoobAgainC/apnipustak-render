...
function load_msg(command) {
    fetch(`json/room/${room}?start=${start}`)
    .then(response => response.json())
    .then(messages => {
        if (first) msgroute.render(<Message messages={messages}/>);
...
function Message(props) {
    ...
    AddMessage = (new_msg, command) => {
        if (command == 'a') setMessage([...messages, new_msg]); 
        else if (command == 'p') setMessage(new_msg.concat(messages));
        else setMessage(new_msg);
    };
    ...
    const msgitem = messages.map((text) => 
        <li className={text.sender == userid ? "replies" : "sent"} key={text.id}>
            <p>{text.message}</p>
        </li>
        );
    return (
        <div className="msginfos">
            <div className="contact-profile">
                <i className="fa-solid fa-arrow-left arrow ps-3 pe-2" onClick={() => history.back()}></i>
                <img src={uPhoto} alt="userimage" />
                <p id="contact-name">{uName}</p> 
                <form className="dots" id="conclude" method="post" action="/delete/" onSubmit={confirmation}>
                            <input type="hidden" name="csrfmiddlewaretoken" value={csrftoken} />
                            <input type="hidden" name="room" value={room}/>
                            <i className="fa-solid fa-ellipsis-vertical px-2" data-bs-toggle="dropdown" style={{cursor: "pointer"}}></i>
						<ul className="dropdown-menu">
                            {owner == userid ? <li> <input name="close" className="dropdown-item" type="submit" value="Close Deal"/> </li> : <li></li>}
                            <li> <input name="cancel" className="dropdown-item text-danger" type="submit" value="Cancel Deal"/></li> 
					    </ul>
            ...
            <div className="messages">
                <ul id="chatmsg" onScroll={(e) => loadMoreMsg(e)}>
